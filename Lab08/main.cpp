...
int main() {
    ...
    for(int i=0;i<=binary.size();i++)
    {
        if(a<4) {
            ...
            a=0;
            h1+=kodowanie(package);
            h3+=kodowanie_SECDED(package);
            package.clear();
            i--;
        }
    ...
    cout<<"Zakodowany:"<<h1<<endl;
    string h2;
    int neg=1;
    h2=negowanie(h1,neg);
    cout<<"Zanegowany "<<neg<<" bit:"<<h2<<endl;
    int a2=0;
    vector<int>package2;
    string decoded;
    int ilosc_paczek=0;
    for(int i=0;i<=h2.size();i++)
    {
        ...
        {
            a2++;
            package2.push_back(int(h2[i]));
        }
        else{
            a2=0;
            decoded+=dekodowanie(package2,ilosc_paczek);
            package2.clear();
            i--;
            ilosc_paczek++;
        }
    }
    cout<<"Odkodowany:"<<decoded<<endl;
    int a3=0;
    ilosc_paczek=0;
    vector<int>package3;
    string decoded_SECDED;
    cout<<"Zakodowany SECDED: "<<h3<<endl;
    h3=negowanie(h3,21);
    h3=negowanie(h3,17);
